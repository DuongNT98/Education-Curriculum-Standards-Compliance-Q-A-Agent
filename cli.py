...
`src/api/server.py`'s existing `secrets_factory(namespace="edu",
agent_name="edu-c2-012")` call shape rather than a per-template value: one
Marketplace Pod deploys exactly one template, so there is no cross-template
secret-path collision to guard against, and `edu` is filled in at
...
if __name__ == "__main__":
    run_agent_marketplace(
        Graph,  # matches src/graph/graph.py's class name (Graph)
        agent_name="edu-c2-012",
        namespace="edu",
